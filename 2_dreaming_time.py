...
caffe_root = '/home/antiantiart/Downloads/deepdream/caffe'
...
def extractVideo(inputdir, outputdir):
    print subprocess.Popen('ffmpeg -i ' + inputdir + ' -f image2 ' + outputdir + '/00%08d.png', shell=True,
...
def main(input, output, image_type, gpu, model_path, model_name, preview, octaves, octave_scale, iterations, jitter, zoom, stepsize, blend, layers, guide_image, start_frame, end_frame, verbose):
    ...
    frame = np.float32(PIL.Image.open(input + '/00%08d.%s' % (frame_i, image_type) ))
    if preview is not 0:
        frame = np.float32(resizePicture(input + '/00%08d.%s' % (frame_i, image_type), preview))
    ...
    for i in xrange(frame_i, nrframes):
        ...
        saveframe = output + "/00%07d.%s" % (frame_i, image_type)
        ...
        newframe = input + "/00%08d.%s" % (frame_i,image_type)
